...
Se o login no frontend falhar com **CORS / Failed to fetch**, confira primeiro os **logs da função** no projeto da API: muitas vezes a causa real é `FUNCTION_INVOCATION_FAILED` (bootstrap Nest, `@sost/shared` ausente no bundle, `MONGODB_URI`/`JWT_SECRET` inválidos, ou dual Express 4+5 com erro `'app.router' is deprecated`). Sem resposta HTTP ok do Nest no preflight, o browser reporta CORS.

O backend usa **Express 5** (alinhado ao Nest 11 / Gestão de Salas), com `overrides` no `pnpm-workspace.yaml` para não misturar majors. O build da API na Vercel também roda `vendor-shared-for-vercel.cjs` para copiar `@sost/shared` para dentro de `backend/node_modules` (o symlink pnpm aponta para fora do Root Directory). Confirme `MONGODB_URI` do Atlas (não localhost) nas env vars de Production.
